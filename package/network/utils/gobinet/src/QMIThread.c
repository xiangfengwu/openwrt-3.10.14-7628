...
int requestSetEthMode(PROFILE_T *profile) {
    ...
    if (linkProto != NULL) {
		...
		dbg_time("%s rawIP=%d; s_9x07=%d", __func__, profile->rawIP, s_9x07);  //bytian
    }
...
#if 1
static void GobiNet_convert_cdma_mcc_2_ascii_mcc( USHORT *p_mcc, USHORT mcc )
{
...
static void GobiNet_convert_cdma_mnc_2_ascii_mnc( USHORT *p_mnc, USHORT imsi_11_12)
{
    ...
    while (remainingLen > 0) {
        ...
        case 0x16: // HDR
            ...
            if(!requestGetHomeNetwork(&cmda_mcc, &cdma_mnc,NULL, NULL) && cmda_mcc) {
                GobiNet_convert_cdma_mcc_2_ascii_mcc(&MobileCountryCode, cmda_mcc);
                GobiNet_convert_cdma_mnc_2_ascii_mnc(&MobileNetworkCode, cdma_mnc);
            }
            break;
        case 0x17: // GSM
            // GSM_SYSTEM_INFO
