...
#ifdef ANDROID
int GobiNet_CM(int argc, char *argv[])
#else
...
__main_loop:
    while (!profile.qmichannel)
    {
        ...
        if (main_loop)
        {
            ...
            dbg_time("Wait for Yuge modules connect");
        ...
        dbg_time("Cannot find qmichannel(%s) usbnet_adapter(%s) for YUGA modules", profile.qmichannel, profile.usbnet_adapter);
        return -ENODEV;
    }
