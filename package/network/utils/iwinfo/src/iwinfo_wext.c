...
int chnl2freq[] ={ 0, 2412, 2417, 2422, 2427, 2432, 2437, 2442, 2447, 2452, 2457, 2462, 2467, 2472, 2484 }; //bytian
...
#endif
	if (ifname!=NULL && strstr(ifname, "apcli") )  //bytian
		*buf = IWINFO_OPMODE_CLIENT;
	else
		*buf = IWINFO_OPMODE_MASTER;
	return 0;
...
static int wext_get_channel(const char *ifname, int *buf)
{
	...
	{
		//printf("bytian,%s: freq.m=%d\n", __FUNCTION__, wrq.u.freq.m);  //bytian
...
static int wext_get_frequency(const char *ifname, int *buf)
{
	...
	if(wext_ioctl(ifname, SIOCGIWFREQ, &wrq) >= 0)
	{
		...
		#endif
			*buf = wext_freq2mhz(&wrq.u.freq);  //bytian
			//printf("byleo,%s: freq.m=%d, *buf=%d\n", __FUNCTION__, wrq.u.freq.m, *buf);  //bytian
			return 0;
		}
		else
		{
...
static int wext_get_mbssid_support(const char *ifname, int *buf)
{
	...
	//return -1;
	*buf = 1;  //support vap; bytian
	return 0;  //support vap; bytian
}
...
#define RT_PRIV_IOCTL 0x8be1
	wrq.u.data.flags   = RT_OID_802_11_PRODUCTID;  //bytian

	int ret = wext_ioctl(ifname, RT_PRIV_IOCTL, &wrq); //bytian
	if( ret>= 0)
		return 0;
...
#define RT_PRIV_IOCTL 0x8be1
	wrq.u.data.flags	= RT_OID_802_11_MANUFACTUREID;	//bytian
